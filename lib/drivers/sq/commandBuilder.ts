...
class CommandBuilderSQ extends CommandBuilder {
    ...
    isPackageForMe(adressRange : IAddressRange, data: Uint8Array | {msb: number, lsb: number}) : Boolean {
        ...
        if(adressRange.msb.from <= dataParsed.msb && adressRange.msb.to >= dataParsed.msb) {
            if(adressRange.lsb.from <= dataParsed.lsb && adressRange.lsb.to >= dataParsed.lsb) {
                return true;
            }
        }
